...
| SWE-bench Verified | Kimi K2.6 · DeepSeek-V4-Pro · MiniMax M2.5 · Grok 4.3 · GLM-5.1 (5개) | Kimi K2.6 · DeepSeek-V4-Pro · MiniMax M2.5 | 실제 repo patch + 공식 Docker 채점에서 상위 3개가 통계적 동률(76/74/74%). |
...
- 작은 함수형 코드 생성/수정: **DeepSeek-V4-Pro · Claude Opus 4.8 · GPT-5.5 · Grok 4.3**이 가장 안정적
- Repository patch(SWE-bench Verified): **Kimi K2.6 · DeepSeek-V4-Pro · MiniMax M2.5** 상위
...
| repository patch / SWE-bench식 task | Kimi K2.6, DeepSeek-V4-Pro, MiniMax M2.5 | Verified 상위 동률 그룹 |
| 비용 민감한 repo patch 반복 실행 | MiniMax M2.5, Grok 4.3 | Verified 해결 건당 비용 최저($0.13~0.14, cache-adj) |
| 두 과제 유형 모두 안정적인 단일 후보 | DeepSeek-V4-Pro | custom 100% + Verified 상위 동률, 두 축 모두 거쳐 일관되게 강함 |

## 다음 단계: 모델 앙상블

이 벤치마크의 결론은 "과제 유형에 따라 강점이 갈린다 — 단일 만능 모델은 없다"는 것입니다. 그렇다면 하나를 고르는 대신 **여러 후보를 돌려 최적 패치를 선택**할 수 있습니다. 이를 PoC로 검증한 후속 작업이 [foundry-coding-ensemble](https://github.com/bskim/foundry-coding-ensemble/blob/main/README_KO.md)입니다.

- 각 모델이 독립적으로 에이전트 루프를 돌려 패치 후보를 생성하고, 집계기가 최적 후보를 선택합니다.
- 집계 전략: ① 실행 기반 선택(build·test·typecheck 자동 평가) ② 비용 인지 라우팅(가장 저렴한 후보부터 시도). 본 저장소가 측정한 모델별 통과율·gross $/resolved 데이터가 이 선택·라우팅 로직의 입력이 됩니다.
- 큐레이션 데이터셋(disc10)에서 실행 기반 선택이 통과율 100%를 달성했습니다(일반화되는 수치는 아님 — PoC의 선택 메커니즘 가능성 검증).
